        ...
        userStore
          ...
          .then(() =>
            toast({
              title: 'Signed in.',
              description:
                "You've  successfully been signed in and your password has been remembered.",
              status: 'success',
              duration: 2500,
            })
          )
...
const SignOutModal: React.FC<Omit<IModal, 'children'>> = ({
  onClose,
  ...props
}) => {
  const userStore = useContext(UserContext);
      ...
      <ModalContent rounded="md">
        ...
        <ModalBody>
          <Stack alignItems="start">
            ...
            {modalErrorMessage && (
              <Text color="red.400">{modalErrorMessage}</Text>
            )}
            <Button
              variant="solid"
              variantColor="cardinalbotics.red"
              isDisabled={!writeUp || !!modalErrorMessage || isModalLoading}
              onClick={() => {
                setModalErrorMessage('');
                ...
                userStore
                  ...
                  .then(() => {
                    ...
                    toast({
                      title: 'Signed out.',
                      description:
                        "You've successfully been signed out and your session has been saved.",
                      status: 'success',
                      duration: 2.5,
                    });
                  })
...
const Account: React.FC<StackProps> = observer((props) => {
  ...
  return (
    <Stack rounded="lg" padding={6} backgroundColor="white" {...props}>
      <Heading
        fontSize="4xl"
        color={userStore.userData.signedIn ? 'green.400' : 'red.400'}
      >
      ...
      <Stat flexGrow={0}>
        ...
        <StatNumber>
          {(userStore.userData.signedIn
            ? moment.duration(
                Math.max(
                  timeStore.currentTime.diff(userStore.userData.lastTime),
                  0
                )
              )
        ...
        <StatHelpText>
          {userStore.userData.signedIn
            ? `Total Time: ${userStore.userData.totalTime.format('hh:mm:ss', {
                trim: false,
              })}`
          ...
          <Button
            ...
            onClick={action(() => {
              ...
              toast({
                ...
                description:
                  "You've successfully forgotten your password and you will no longer be automatically signed in.",
                status: 'success',
                duration: 2500,
              });
            })}
...
const SessionTableRow: React.FC<FlexProps & { session: Session }> = ({
  session,
  ...props
}) => {
    ...
    <Flex direction="row" {...props}>
      ...
      <Text flexBasis="70%" wordBreak="break-all" paddingX={6} paddingY={2}>
        {session.did && session.did.length > 250 ? (
          <Fragment>
            {!isExpanded
              ? session.did.slice(0, 250).concat('...')
              : session.did}
            <Text
              cursor="pointer"
              color="cardinalbotics.red.400"
    ...
    <Flex direction="column" {...props}>
      ...
      {userStore.userData.sessions.reverse().map((session, idx) => (
        <SessionTableRow
          ...
          session={session}
          backgroundColor={
            session.flagged ? 'red.300' : idx % 2 == 0 ? 'gray.100' : 'white'
          }
        />
      ))}
    </Flex>
      ...
      {userStore.otherUserData
        ...
        .map((user, idx) => (
          ...
          >
            ...
            <Text flexBasis="33%" paddingX={6} paddingY={3}>
              {moment
                .duration(timeStore.currentTime.diff(user.lastTime))
                .format('hh:mm:ss', {
                  minValue: 0,
                  trim: false,
                })}
...
const HomePage: React.FC = () => {
  ...
  return useObserver(() => (
    ...
    >
      ...
      <Stack
        alignItems="start"
        width={!userStore.userData ? ['80%', '65%', '50%'] : ['90%', '80%']}
        flexGrow={1}
        spacing={4}
      >
        <Box flexGrow={1} />
